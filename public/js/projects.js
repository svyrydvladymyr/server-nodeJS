...
//for show widget skills
    let showProjectsList = (res) => {
        ...
        SE.$('edit-projects-wrap').innerHTML = `<i class='far fa-edit' id="edit-skills" onclick='SE.send({"userid":window.location.href}, "/showprojects", WPROJ.showEditProjectsList);'></i>`;
...
//for show edit in widget projects
    let showEditProjectsList = (res) => {
        SE.$('edit-projects-wrap').innerHTML = ``;
        let parseObj = JSON.parse(res);
        ...
        if (kilkprojects <= 10){
            ...
            <div class="skills-wrap-form" style="max-width:100%; width:100%; background:#f1f1f1; padding: 15px 0px 0px 0px; border: 1px solid #9e9e9e;">   
                <div class="skills-boks">
                    ...
                    <p id="mess-addproj" style="width:100%; text-align:center; color:#b80000; height:20px;"></p>
                    </div>
                </div> 
            </div>`;
        }
...
//for show edit form for single project
    let showEditForm = (el) => {
        let opendelform = document.getElementsByClassName('projects-confirm-box').length;
        if (opendelform === 0){
