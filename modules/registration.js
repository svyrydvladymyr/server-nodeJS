...
let registrationUsers = (req, res) => {     
    // let parseForTry = JSON.parse(req.objreg);
    // console.log(req);
    // console.log( "ggggg", req.body.login);
    // console.log( "ggggg", req.body.objreg);
    // console.log(parseForTry);

    // let sql = `SELECT login email FROM customers WHERE login = ${parseForTry.login}`;
    ...
    upload(req, res, (err) => {
        if (err) {
            ...
            console.log("objClient---", JSON.parse(req.body.objreg));
            console.log("file----", req.file);

            prUs.userid = translit(`${parseObjUsers.name}${parseObjUsers.surname}`).toLowerCase() + '-' + token(10);
            if (req.file !== undefined){
                prUs.ava = req.file.filename;
                checkObjValues("^[^a-zA-Z-_]+$", "avasettings", "Bad ava parameters!", parseObjUsers, res);
            } else {
                prUs.ava = '';
                prUs.avasettings = '';
            }
