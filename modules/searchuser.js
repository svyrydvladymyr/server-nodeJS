...
let showfriends = (req, res) => {
    ...
    let sqlkilk = `SELECT S.friendid FROM friends_${userid} S INNER JOIN users U ON U.userid=S.userid WHERE S.userid = '${userid}' AND S.friendstatus = '${typesearch}' ORDER BY S.friendid ASC, S.friendvisit + 0 DESC;`;
    let sqlsel = `SELECT U.userid, U.name, U.surname, U.surname, U.regtype, S.friendid, S.friendvisit, S.friendstatus FROM friends_${userid} S INNER JOIN users U ON U.userid=S.userid WHERE S.userid = '${userid}' AND S.friendstatus = '${typesearch}' ORDER BY S.friendid ASC, S.friendvisit + 0 DESC LIMIT ${step} ${limitsearch};`;
